...
DECLARE
    ...
    cyclingId INT;
BEGIN
    SELECT id INTO meyerId
    FROM t_person
    WHERE first_name = 'Christopher' AND last_name = 'Meyer';
    
    SELECT id INTO birnbaumId
    FROM t_person
    WHERE first_name = 'Hildegart' AND last_name = 'Birnbaum';
    ...
    VALUES
        (nextval('vacation_pk_seq'), meyerId, cyd(1,1), cyd(1,10), 'Holiday'),
        (nextval('vacation_pk_seq'), meyerId, cyd(5,1), cyd(5,1), 'Eschborn-Frankfurt'),
        ...
        (nextval('vacation_pk_seq'), meyerId, nyd(5,1), nyd(5,1), 'Eschborn-Frankfurt'),
    ...
    INSERT INTO t_vacation(id, person_fk, start_date, end_date, comment)
    VALUES
        (nextval('vacation_pk_seq'), birnbaumId, cyd(1,1), cyd(1,10), 'Holiday'),
